...
namespace mod::common
{
    ...
    AttackParams *kSuperHammerRecoilParams = reinterpret_cast<AttackParams *>(0x80353F68);
    AttackParams *kUltraHammerRecoilParams = reinterpret_cast<AttackParams *>(0x803540E8);
    ...
    AttackParams *kHammerThrowParams = reinterpret_cast<AttackParams *>(0x80354C28);
    AttackParams *kPiercingBlowParams = reinterpret_cast<AttackParams *>(0x80354CE8);
    AttackParams *kSuperChargeParams = reinterpret_cast<AttackParams *>(0x80355228);
    AttackParams *kSuperChargePartnerParams = reinterpret_cast<AttackParams *>(0x803552E8);
    ...
    const int32_t kJinModuleAtomicBooItemObjectFlagOffset = 0xD7D0;
    const int32_t kJinModuleAtomicBooItemGswfSetOffset = 0xD7DC;
    ...
    const char *GetCurrentArea()
    {
        if (!*kSaveDataPtr)
            return nullptr;
        return reinterpret_cast<const char *>(reinterpret_cast<uintptr_t>(*kSaveDataPtr + kCurrentAreaOffset));
    }
    ...
    const char *GetCurrentMap()
    {
        if (!*kSaveDataPtr)
            return nullptr;
        return reinterpret_cast<const char *>(reinterpret_cast<uintptr_t>(*kSaveDataPtr + kCurrentMapOffset));
    }
    ...
    const char *GetFilename()
    {
        if (!*kSaveDataPtr)
            return nullptr;
        return reinterpret_cast<const char *>(reinterpret_cast<uintptr_t>(*kSaveDataPtr + kFilenameOffset));
    }
    ...
    bool GetSavedFlagValue(int32_t flag)
    {
        if (!*kSaveDataPtr)
            return false;
        int32_t flag_word = *reinterpret_cast<int32_t *>(*kSaveDataPtr + kSavedFlagsOffset + (flag >> 5) * sizeof(uint32_t));
        return flag_word & (1U << (flag & 31));
    }

    bool HasKeyItemInInventory(int16_t item_id)
    {
        int16_t *key_items_arr = reinterpret_cast<int16_t *>(*kPouchPtr + kKeyItemsOffset);
        for (int i = 0; i < 121; ++i)
        {
            if (*key_items_arr++ == item_id)
                return true;
        }
        return false;
    }
    ...
    {
        if (!*kUnknownStatusWindowStructPtr)
            return 0.0f;
        float *height = reinterpret_cast<float *>(*kUnknownStatusWindowStructPtr + kStatusWindowHeightOffset);
        return *height;
    }
} // namespace mod::common
