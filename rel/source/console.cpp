...
namespace mod
{
    ...
    ConIntVar con_mono("con_mono",
                       1,
                       [](ConIntVar *self, int new_value)
                       {
    ...
    void ConsoleSystem::updateUsbGecko()
    {
        ...
        if (!ugProbe(chan))
        {
            if (con_ug_mode.value == 1)
            {
                ...
                logWarning("Console UG enabled but none attached to slot %c, switching to standby\n", 'A' + chan);
                con_ug_mode.value = 2;
            }
            return;
        }
        ...
        while (true)
        {
            ...
            *cmd_end = '\0';
            ...
            memmove(mUgBuffer, mUgBuffer + cmd_size, mUgBufferSize - cmd_size);
    ...
    void ConsoleSystem::processCommand(const char *text)
    {
        ...
        for (ConCommand *cc = ConCommand::sFirst; cc; cc = cc->next)
        {
            if (strlen(cc->name) == (size_t)ident_len && !strncmp(ident_start, cc->name, ident_len))
            {
        ...
        for (ConIntVar *cv = ConIntVar::sFirst; cv; cv = cv->next)
        {
            if (strlen(cv->name) == (size_t)ident_len && !strncmp(ident_start, cv->name, ident_len))
            {
                int new_value;
