...
namespace mod
{
    ConCommand perf_fake_spike("perf_fake_spike",
                               [](const char *args)
                               {
                                   ...
                                   if (sscanf(args, "%" PRIu32, &durationMs) == 1)
                                   {
                                       ...
                                       do
                                       {
                                       } while (gc::os::OSGetTick() - startTick < durationTicks);
                                   }
                               });
    ...
    MOD_UPDATE_FUNCTION()
    {
        ...
        if (!valid)
        {
            gConsole->overlay("Heap: %d corrupt at %08lx\n", heapId, reinterpret_cast<uint32_t>(currentChunk));
            return;
        }
        ...
        gConsole->overlay("Heap: id %d, %.2f/%.2fkb, %d cks\n", heapId, usage / 1024.f, heap.capacity / 1024.f, chunks);
    }
} // namespace mod
