...
extern int32_t setting_sun_stone[];
extern int32_t sun_stone_set_event[];
extern int32_t setting_moon_stone[];
extern int32_t moon_stone_set_event[];
...
EVT_BEGIN(stones_evt)
    IF_EQUAL(GSWF(1776), 0)
        ...
        IF_LARGE_EQUAL(LW(0), 1)
            USER_FUNC(evt_case::evt_run_case_evt, 9, 1, PTR("a_isi_4"), 0, PTR(&sun_stone_set_event), 0)
        END_IF()
    ELSE()
        RUN_CHILD_EVT(&setting_sun_stone)
    END_IF()
    ...
    IF_EQUAL(GSWF(1777), 0)
        ...
        IF_LARGE_EQUAL(LW(0), 1)
            USER_FUNC(evt_case::evt_run_case_evt, 9, 1, PTR("a_isi_3"), 0, PTR(&moon_stone_set_event), 0)
        END_IF()
    ELSE()
        RUN_CHILD_EVT(&setting_moon_stone)
    END_IF()
    RETURN()
EVT_END()
...
namespace mod
{
    void main()
    {
        ...
        gonbaba_evt[19] = EVT_HELPER_CMD(2, 24);
        gonbaba_evt[21] = 0;
