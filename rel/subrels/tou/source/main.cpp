...
EVT_BEGIN(tou_party_evt)
    IF_EQUAL(GSWF(6079), 0) // skip the spawn once collected (get_item would hang)
...
EVT_BEGIN(tou_05_talk_gans_evt)
    ...
    IF_LARGE_EQUAL(LW(0), 0)
        ...
        END_SWITCH()
        IF_EQUAL(LW(2), 0)
            USER_FUNC(evt_msg::evt_msg_print_add, 0, PTR("grubba_pay_free"))
...
EVT_BEGIN(tou_talk_kinoshikowa_evt)
    USER_FUNC(evt_mario::evt_mario_key_onoff, 0)
    // Insert BGM here
    USER_FUNC(evt_msg::evt_msg_print, 0, PTR("jolene_bribe"), 0, PTR(&jolene))
    USER_FUNC(tou_evt_tou_get_ranking, LW(2))
    SET(GSW(1724), LW(2))
    IF_SMALL(GSWF(6075), 1)
        SET(LW(14), PTR("<numselect 11 20 20 1>\n %" PRId32)) // Min, Max, Start, Step
    ...
    IF_LARGE_EQUAL(LW(0), 0)
        ...
        USER_FUNC(tou_evt_tou_get_ranking, LW(2))
        IF_EQUAL(LW(2), LW(1))
            USER_FUNC(evt_msg::evt_msg_print_add, 0, PTR("jolene_rank_same"))
            USER_FUNC(evt_mario::evt_mario_key_onoff, 1)
            RETURN()
        END_IF()
        SUB(LW(2), LW(1))
        SET(LW(3), GSW(1728)) // shared bribeCost setting
        SET(LW(4), LW(2))
        MUL(LW(2), LW(3))
        IF_SMALL(LW(2), 0)
            MUL(LW(2), -1)
        ...
        END_SWITCH()
        IF_EQUAL(LW(2), 0)
            USER_FUNC(evt_msg::evt_msg_print_add, 0, PTR("jolene_pay_free"))
            USER_FUNC(evt_msg::evt_msg_select, 0, PTR("jolene_pay_prompt"))
            IF_EQUAL(LW(0), 0)
                USER_FUNC(evt_msg::evt_msg_print_add, 0, PTR("jolene_pay_accept_free"))
                USER_FUNC(setRanking, LW(1))
                USER_FUNC(evt_mario::evt_mario_key_onoff, 1)
                RETURN()
            ELSE()
                USER_FUNC(evt_msg::evt_msg_print_add, 0, PTR("jolene_pay_accept_no"))
                USER_FUNC(evt_mario::evt_mario_key_onoff, 1)
                RETURN()
            END_IF()
        END_IF()
        USER_FUNC(evt_window::evt_win_coin_on, 0, LW(10))
        USER_FUNC(evt_msg::evt_msg_fill_num, 0, LW(14), PTR("jolene_pay"), LW(2))
        USER_FUNC(evt_msg::evt_msg_print_add, 1, LW(14))
        USER_FUNC(evt_msg::evt_msg_select, 0, PTR("jolene_pay_prompt"))
        IF_EQUAL(LW(0), 0)
            USER_FUNC(evt_pouch::evt_pouch_get_coin, LW(3))
            IF_SMALL(LW(3), LW(2))
                USER_FUNC(evt_msg::evt_msg_print_add, 0, PTR("jolene_no_coins"))
                USER_FUNC(evt_window::evt_win_coin_off, LW(10))
            ELSE()
                MUL(LW(2), -1)
                USER_FUNC(evt_pouch::evt_pouch_add_coin, LW(2))
                USER_FUNC(evt_msg::evt_msg_print_add, 0, PTR("jolene_pay_accept"))
                USER_FUNC(evt_window::evt_win_coin_off, LW(10))
                USER_FUNC(setRanking, LW(1))
            END_IF()
        ELSE()
        ...
        END_IF()
    ELSE()
        USER_FUNC(evt_msg::evt_msg_print_add, 0, PTR("jolene_pay_reject"))
    END_IF()
    // Revert BGM here
    USER_FUNC(evt_mario::evt_mario_key_onoff, 1)
    RETURN()
...
EVT_BEGIN(tou_talk_kinoshikowa_hook)
	RUN_CHILD_EVT(tou_talk_kinoshikowa_evt)
    RETURN()
EVT_END()
